...
enum ServerToolArgs {
    ...
    nonisolated static func firstObjectEnd(of text: String) -> String? {
        ...
        for i in text.indices {
            scanner.scan(text[i])
            if scanner.isComplete { return String(text[text.startIndex...i]) }
        }
        return nil
    }
}

/// JSON 객체 경계 스캐너 (T-360 분리): 문자열 따옴표·이스케이프·중괄호 중첩을 추적.
/// 순수 상태 기계라 순환 복잡도가 `firstObjectEnd`에서 분리된다.
private struct JSONObjectScanner {
    ...
    mutating func scan(_ c: Character) {
        if inString {
            scanString(c)
            return
        }
    ...
    private mutating func scanString(_ c: Character) {
        if escaped {
            escaped = false
        } else if c == "\\" {
            escaped = true
