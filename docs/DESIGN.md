...
* 좌측 사이드바 (220px): 상단 탭 [채팅 | 모델] (T-230, 영속).
  ...
  스테이징 `~/.litert-lm-studio/staging`(T-314 홈 통합), 완료 후에도 보관. 목록 캐시 60초.
  ...
  - 엔진 행 (규칙 1, T-231 후속): 입력창 route 추종. 앱 내 엔진=초기화/중지/다시 실행, 서버=데몬 시작/중지.
   ...
   - 대화 목차 플로팅 (T-258, PLAN_v54): 채팅 우측 중앙 바 3개→호버 확장,
     질문 첫 줄 목록+점프·플래시, 설정 채팅 탭 토글.
  - 새 채팅 드래프트 (T-137): 버튼 클릭은 방을 만들지 않고 틴트+입력 포커스만. 첫 전송 시 방 생성·선택. 평상시 버튼은 fill 없음.
* 중앙 (가변): 채팅 타임라인 + 스트리밍 버블 + 중단 버튼 + 입력창(첨부·경로 메뉴·전송). 빈 화면은 웰컴.
  - 웰컴 (T-262, PLAN_v57; T-294 2원 합류, PLAN_v83): 환영 헤더+환경 1줄(설치·최신 버전)+새소식 카드 3건(엔진+앱 What's New 요약·
     자세히 링크·새로고침, 업데이트 판정은 엔진만)+신버전 시 업데이트 행(확인 팝업→`uv tool upgrade`)+추천 링크 5행
     (공식 문서·GitHub·LiteRT-LM Studio 앱 저장소·Hugging Face·AI Edge Gallery, 기본 브라우저)+서버 미실행 축소 안내.
   - 시스템 현황 (T-293, PLAN_v82): LIVE 행+CPU 2선·RAM 3선·GPU. 데몬 히어로 카드 삭제 (샘플링·E-MAC-NET-0010 유지).
  ...
  - 터미널: 탭 전환 고정 (콘텐츠 영역 통일+셀 늘림+헤더 자리 유지) (T-100).
   - 버블: 유저 우측 / 어시스턴트 좌측 + 복사·재시도 푸터 + 에러 테두리 (PLAN_v3).
     - 후속질문 칩 (T-261, PLAN_v56; T-291 LLM 전환, PLAN_v80; T-292 선행, PLAN_v81):
       마지막 완료 응답 아래 우측 정렬 3개 캡슐, 클릭 즉시 전송, 설정 채팅 탭 토글.
       서버 경로는 스트리밍 중 300자 도달 시 선행 호출+완료 시 드리프트 판정(유지/재호출),
       앱 내 엔진은 완료 후 즉시 호출 (Engine 동시 추론 미검증). 프롬프트 Q 1000·A 800자,
       실패 시 기존 휴리스틱 폴백.
   - 생각·도구 표시 (T-266 S-1, PLAN_v61): 어시스턴트 버블 상단 생각 접기 회색 블록+
     도구 호출 칩 (이름·인자 요약·상태)+푸터 🔧횟수. 실행은 S-2/S-3.
   - 도구 실행 (T-267 S-2, PLAN_v61): 로컬 2종(현재시각·사칙계산)+Ask 인라인 승인 팝업+
...
- MCP·스킬 (T-285, PLAN_v73 → PLAN_v93): 외부 서버(stdio·SSE) 연결+게이트웨이 2종,
      ...
      멀티 루트(`skillRoots`) 스캔+우선순위(opencode > claude > agents) 병합. skillsTab 행에 출처 뱃지 표시.
  ...
  - 본문: 네이티브 마크다운 (T-150, WKWebView 제거): 줄블록(제목·목록·표·구분선·인용·문단)+펜스 코드(Highlightr 색상·헤더·복사)+줄바꿈 보존. 대화 열 12px inset (T-172).
   ...
   - 명령 팔레트 (T-263, PLAN_v58): Spotlight식 중앙 상단 플로팅 (시트 교체).
     검색창+명령 필터 6종+전체 채팅 검색 8건 (방 이름·미리보기·시각).
     ↑↓·Enter·Esc, ⌘K 열기. 채팅 Enter는 방 전환+스크롤+1.5초 플래시 (전송 중 제외).
