...
* 중앙 (가변): 채팅 타임라인 + 스트리밍 버블 + 중단 버튼 + 입력창(첨부·경로 메뉴·전송). 빈 화면은 웰컴.
  - 웰컴 (T-262, PLAN_v57): 환영 헤더+환경 1줄(설치·최신 버전)+새소식 카드 3건(What's New 요약·
    자세히 링크·새로고침)+신버전 시 업데이트 행(확인 팝업→`uv tool upgrade`)+추천 링크 4행
    (공식 문서·GitHub·Hugging Face·AI Edge Gallery, 기본 브라우저)+서버 미실행 축소 안내.
  ...
  - 터미널: 탭 전환 고정 (콘텐츠 영역 통일+셀 늘림+헤더 자리 유지) (T-100).
   - 버블: 유저 우측 / 어시스턴트 좌측 + 복사·재시도 푸터 + 에러 테두리 (PLAN_v3).
    - 후속질문 칩 (T-261, PLAN_v56): 마지막 완료 응답 아래 우측 정렬 3~4개 캡슐,
      클릭 즉시 전송, 설정 채팅 탭 토글. 로컬 휴리스틱 (LLM 호출 없음).
