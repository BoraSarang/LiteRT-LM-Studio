...
* [ ] T-362 키 체계 + 순수 로직 전환: `ToolCatalog`·`ModelCatalog`·`ModelAlias`·`TimeFormat`·`MenuBarStatusText` 등 문자열 반환 로직을 키/언어 주입으로 바꾸고, 테스트 단언 한국어 225줄을 키 기반으로 교체. `DS*` 컴포넌트 `String` 파라미터 수용 방식 확정
* [ ] T-363 화면 문구 치환: 사용자 UI 리터럴(약 401건)을 `L(L10n.…)`/`T`/`LK`로 교체하며, 치환 시 기존 한국어를 카탈로그 ko 값으로 옮겨 적는다
* [ ] T-364 카탈로그 ko/en 확정: `Localizable.xcstrings`에 전 키 ko 값 + 영어 번역 채우기(커버리지 테스트로 누락 차단)
* [ ] T-365 한국어 문구 다듬기: korean-humanizer로 긴 문장·라벨 검토(뜻 보존, 개발자 용어 유지). 로그·디버그 패널은 대상 제외
* [ ] T-366 시스템 표면 + 문서: `InfoPlist.xcstrings`(권한 설명 4건), `error_message_ko.json`(E-MAC-*) 카탈로그 이관, 창/메뉴 제목 즉시 갱신 실측, README·CHANGELOG·릴리스 노트 언어 안내
