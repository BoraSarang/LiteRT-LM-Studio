...
3. T-353: Exa 기본 캐시가 GitHub 릴리스 페이지를 v0.16.1에서 멈춤(실측, 실제 v0.17.1) → `web_fetch`(contents)에 `maxAgeHours:0` 라이브 크롤. 사용자 눈확인 통과. T-354로 검색은 캐시 허용(비용 절충)+`[웹 결과 활용 규칙]` 시스템 블록(1턴·재전송 턴 모두).
4. T-351: 시스템 프롬프트 `[도구 병렬 규칙]`+본문 `parallel_tool_calls: true`(기존 runTurnCalls 루프 재사용). curl 1응답 멀티콜 + UI 눈확인 완료.
...
7. 전부 눈확인 완료: T-351 병렬, T-353/T-354 웹 최신성·본문 답변. 남은 후보: Exa 키 Keychain 이관(사용자: 그대로 둠).
